...
_SettingValueT = TypeVar("_SettingValueT")
...
def require_setting(value: _SettingValueT | None, name: str) -> _SettingValueT:
    """Return a configured value, or fail explicitly at its point of use."""
    if value is None:
        raise ImproperlyConfigured(f"{name} not set")
    return value
...
class Settings:
    ...
    BACKEND_TYPE: str | None = field(default_factory=get_settings("BACKEND_TYPE"))
    # 验证用户登录态的 API，如 蓝鲸统一登录校验登录态的 API
    USER_COOKIE_VERIFY_URL: str | None = field(default_factory=get_settings("USER_COOKIE_VERIFY_URL"))

    # 是否使用多租户模式
    ENABLE_MULTI_TENANT_MODE: bool = field(default_factory=get_settings("ENABLE_MULTI_TENANT_MODE", default=False))
    # 验证用户信息的网关 API
    USER_INFO_APIGW_URL: str | None = field(default_factory=get_settings("USER_INFO_APIGW_URL"))

    # 获取用户详情的 API，如中文名、邮箱等，且必须提供应用鉴权信息
    TOKEN_USER_INFO_ENDPOINT: str | None = field(default_factory=get_settings("TOKEN_USER_INFO_ENDPOINT"))
    TOKEN_APP_CODE: str | None = field(default_factory=get_settings("TOKEN_APP_CODE"))
    TOKEN_SECRET_KEY: str | None = field(default_factory=get_settings("TOKEN_SECRET_KEY"))
    ...
    REQUESTS_CERT: str | None = field(default_factory=get_settings("REQUESTS_CERT"))
