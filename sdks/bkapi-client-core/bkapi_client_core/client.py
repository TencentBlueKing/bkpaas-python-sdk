...
class RequestContextBuilder(object):
    def build(self, endpoint: str, operation_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def build_url(self, context: Dict[str, Any], endpoint: str, path: str):
        context["url"] = urljoin(endpoint, path)

    def build_data(self, context: Dict[str, Any], data: Optional[Dict[str, Any]] = None):
        if not data:
            return
...
class ResponseHeadersRepresenter(object):
    ...
    @property
    def has_apigateway_error(self) -> bool:
...
class BaseClient(object):
    ...
    def check_response_apigateway_error(self, response: Optional[Response]) -> None:
    ...
    def _get_request_context(self, operation: Operation, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _handle_exception(
        ...
        if isinstance(exception, ResponseError):
            logger.warning("%s\n%s", str(exception), CurlRequest(exception.request))  # type: ignore
            ...
            logger.exception(
                ...
                CurlRequest(exception.request),  # type: ignore
            )
        else:
    ...
    def _handle_response_content(self, operation: Operation, response: Optional[Response]):
        if response is None:
            return None
