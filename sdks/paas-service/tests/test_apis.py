...
from paas_service.models import Plan, ProvisionRecord, Service, ServiceInstanceConfig, SpecDefinition, Specification
...
class TestServiceManageViewSet:
    def test_list(self, rf, service, plan, platform_client):
        ...
        assert service_data["plan_schema"] == {
            "properties": {"foo": {"description": "这是一个示例字段", "title": "Foo", "type": "string"}},
            "required": ["foo"],
            "title": "DummySchema",
            "type": "object",
        }
...
class TestSvcInstanceViewSet:
    ...
    def test_idem_prov(self, rf, service, plan, platform_client):
        ...
        request = rf.post(
            f"/services/{service.uuid}/instances/",
            data=json.dumps(
                {
        ...
        request2 = rf.post(
            f"/services/{service.uuid}/insntaces/",
            data=json.dumps(
                {
                    "params": {"engine_app_name": "test-1"},
                    "plan_id": str(plan.uuid),
