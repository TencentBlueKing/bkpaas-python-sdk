...
@pytest.fixture()
def public_key_secret(api_name, public_key):
    return V1Secret(
        ...
        data={
            api_name: base64.b64encode(public_key.encode()).decode(),
        },
    )
...
class TestSecretPublicKeyManager:
    ...
    def test_get_secret_not_exists(self, issuer, api_name):
        self.k8s_client.read_namespaced_secret.side_effect = exceptions.ApiException(404)
        assert self.manager.get(api_name, issuer) is None
    ...
    def test_get(self, issuer, api_name, public_key_secret):
        self.k8s_client.read_namespaced_secret.return_value = public_key_secret
        assert self.manager.get(api_name, issuer)

    def test_set_for_create(self, issuer, api_name, public_key):
        self.k8s_client.patch_namespaced_secret.side_effect = exceptions.ApiException(404)
        self.manager.set(api_name, public_key, issuer)

        assert self.k8s_client.patch_namespaced_secret.call_count == 1
        assert self.k8s_client.create_namespaced_secret.call_count == 1

    def test_set_for_update(self, issuer, api_name, public_key):
